...
import type { FormatAdapter, FormatSnapshot, PdfPageContent } from './formatAdapter';
...
async function snapshotPdf(sourceUrl: string): Promise<FormatSnapshot | null> {
  ...
  if (coverBlob) {
    await putFiles(sourceUrl, [{ relativePath: PDF_COVER_CACHE_FILE_KEY, blob: coverBlob }]);
  }
...
async function openPdf(id: string, config: BookConfig): Promise<PdfBookRecord | null> {
  try {
